...
SELECT 
    t.store_id, s.store_name,s.city, t.total_revenue
FROM
    (SELECT 
        o.store_id,
...
with tab as 
    (select t2.store_id,staff_name, total_revenue , 
    rank() over( partition by t2.store_id order by total_revenue desc) as ranks 
from
    (select o.staff_id,o.store_id,concat(s.first_name,' ',s.last_name) as staff_name, 
    ROUND(SUM(oi.quantity * oi.list_price), 2) as total_revenue 
from orders as o 
join order_items as oi on o.order_id = oi.order_id 
...
SELECT store_id, staff_name, total_revenue FROM tab WHERE ranks = '1';
    
-- Generate a report showing year-over-year sales growth for the last 3 years.

select 
distinct year(t.order_date) as year, 
round(sum(t.total_revenue) over( partition by year(t.order_date)),2) 
as year_over_year_sales 
from 
(select o.order_date, ROUND(SUM(oi.quantity * oi.list_price), 2) as total_revenue 
from orders as o join order_items as oi 
on oi.order_id=o.order_id group by o.order_date) as t;

-- Use a window function to rank products based on total revenue and 
-- display the top n products using stored procedures.

create procedure top_n_products(in n int)
select t.product_name,t.total_revenue, 
rank() over(order by t.total_revenue desc) as rank_no 
from
(select p.product_name, round(sum(oi.list_price*oi.quantity),2) 
as total_revenue 
...
select t.customer_id , t.average_order_value, 
dense_rank() over ( order by t.average_order_value desc) as ranks 
from
(select o.customer_id , round(avg(oi.list_price*oi.quantity),2) 
...
SELECT 
distinct DATE_FORMAT(order_date,'%Y-%m') AS month, 
round(SUM(oi.quantity *oi.list_price) 
OVER (ORDER BY DATE_FORMAT(order_date,'%Y-%m')),0)AS cumulative_revenue 
FROM orders o JOIN order_items oi ON
o.order_id=oi.order_id where DATE_FORMAT(order_date,'%Y-%m') between '2016-01' and '2016-12';
...
-- finding the top 5 cities that produce high sales revenue

create procedure top_n_cities(in a int)
select c.city , 
round(sum(oi.list_price*oi.quantity)) as revenue  
from customers as c join orders as o on
c.customer_id=o.customer_id join order_items as oi 
on o.order_id=oi.order_id 
group by c.city order by revenue desc limit a;

call top_n_cities(5);

-- bottom n cities

create procedure bottom_n_cities(in a int)
select c.city , 
round(sum(oi.list_price*oi.quantity)) as revenue  
from customers as c join orders as o on
c.customer_id=o.customer_id join order_items as oi 
on o.order_id=oi.order_id 
group by c.city order by revenue limit a;

call bottom_n_cities(5);
